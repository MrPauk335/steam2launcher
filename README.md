...
- Инкрементальные обновления игры: если в `games.json` указан `Repo` (owner/repo),
  кнопка «Обновить» скачивает delta-патч из последнего релиза GitHub и накатывает его
  поверх установленной сборки (старые файлы удаляются по манифесту). База (полный архив)
  остаётся на основном хостинге (MediaFire), патчи — только изменённые файлы.
